 /*                                                                      
 Copyright 2019 Blue Liang, liangkangnan@163.com
                                                                         
 Licensed under the Apache License, Version 2.0 (the "License");         
 you may not use this file except in compliance with the License.        
 You may obtain a copy of the License at                                 
                                                                         
     http://www.apache.org/licenses/LICENSE-2.0                          
                                                                         
 Unless required by applicable law or agreed to in writing, software    
 distributed under the License is distributed on an "AS IS" BASIS,       
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and     
 limitations under the License.                                          
 */

`include "khoree_defines.vh"

// ????
// ???????
module khoree_ex(

    input wire clk,
    input wire rst,

    // from id
    input wire[`KHOREE_InstBus] inst_i,            // ????
    input wire[`KHOREE_InstAddrBus] inst_addr_i,   // ????
    input wire reg_we_i,                    // ????????
    input wire[`KHOREE_RegAddrBus] reg_waddr_i,    // ????????
    input wire[`KHOREE_RegBus] reg1_rdata_i,       // ?????1????
    input wire[`KHOREE_RegBus] reg2_rdata_i,       // ?????2????
    input wire[`KHOREE_MemAddrBus] op1_i,
    input wire[`KHOREE_MemAddrBus] op2_i,
    input wire[`KHOREE_MemAddrBus] op1_jump_i,
    input wire[`KHOREE_MemAddrBus] op2_jump_i,
    input wire prdt_taken_i,

    // from mem
    input wire[`KHOREE_MemBus] mem_rdata_i,        // ??????

    // to mem
    output reg[`KHOREE_MemBus] mem_wdata_o,        // ?????
    output reg[`KHOREE_MemAddrBus] mem_raddr_o,    // ?????
    output reg[`KHOREE_MemAddrBus] mem_waddr_o,    // ?????
    output wire mem_we_o,                   // ??????
    output wire mem_req_o,                  // ????????
    input  wire mem_ack_i,

    // to regs
    output wire[`KHOREE_RegBus] reg_wdata_o,       // ??????
    output wire reg_we_o,                   // ?????????
    output wire[`KHOREE_RegAddrBus] reg_waddr_o,   // ????????

    // to ctrl
    output wire hold_flag_o,                // ??????
    output wire jump_flag_o,                // ??????
    output wire[`KHOREE_InstAddrBus] jump_addr_o,  // ??????

    // to pipeline registers
    output wire stall

    );

    wire[1:0] mem_raddr_index;
    wire[1:0] mem_waddr_index;
    wire[31:0] sr_shift;
    wire[31:0] sri_shift;
    wire[31:0] sr_shift_mask;
    wire[31:0] sri_shift_mask;
    wire[31:0] op1_add_op2_res;
    wire[31:0] op1_jump_add_op2_jump_res;
    wire op1_ge_op2_signed;
    wire op1_ge_op2_unsigned;
    wire op1_eq_op2;
    wire[6:0] opcode;
    wire[2:0] funct3;
    wire[6:0] funct7;
    wire[4:0] rd;
    reg[`KHOREE_RegBus] reg_wdata;
    reg reg_we;
    reg[`KHOREE_RegAddrBus] reg_waddr;
    reg hold_flag;
    reg jump_flag;
    reg[`KHOREE_InstAddrBus] jump_addr;
    reg mem_we;
    reg mem_req;

    assign opcode = inst_i[6:0];
    assign funct3 = inst_i[14:12];
    assign funct7 = inst_i[31:25];
    assign rd = inst_i[11:7];

    assign sr_shift = reg1_rdata_i >> reg2_rdata_i[4:0];
    assign sri_shift = reg1_rdata_i >> inst_i[24:20];
    assign sr_shift_mask = 32'hffffffff >> reg2_rdata_i[4:0];
    assign sri_shift_mask = 32'hffffffff >> inst_i[24:20];

    assign op1_add_op2_res = op1_i + op2_i;
    assign op1_jump_add_op2_jump_res = op1_jump_i + op2_jump_i;

    // ??????
    assign op1_ge_op2_signed = $signed(op1_i) >= $signed(op2_i);
    // ??????
    assign op1_ge_op2_unsigned = op1_i >= op2_i;
    assign op1_eq_op2 = (op1_i == op2_i);

    assign mem_raddr_index = (reg1_rdata_i + {{20{inst_i[31]}}, inst_i[31:20]}) & 2'b11;
    assign mem_waddr_index = (reg1_rdata_i + {{20{inst_i[31]}}, inst_i[31:25], inst_i[11:7]}) & 2'b11;

    assign reg_wdata_o = reg_wdata;
    // ????????????
    assign reg_we_o = reg_we;
    assign reg_waddr_o = reg_waddr;

    assign mem_we_o = mem_we;
    assign mem_req_o = mem_req;

    assign hold_flag_o = `KHOREE_HoldDisable;
    assign stall = hold_flag;               // ?????
    assign jump_flag_o = jump_flag;
    assign jump_addr_o = jump_addr;

    reg mem_ack_flag;
    always @ (posedge clk) begin
        if (rst == `KHOREE_RstEnable) begin
            mem_ack_flag <= 0;
        end else if(mem_req && mem_ack_i) begin
            mem_ack_flag <= 1;
        end else
            mem_ack_flag <= 0;
    end


    // ??
    always @ (*) begin
        reg_we = reg_we_i;
        reg_waddr = reg_waddr_i;
        reg_wdata = `KHOREE_ZeroWord;
        mem_req = `KHOREE_RIB_NREQ;
        mem_wdata_o = `KHOREE_ZeroWord;
        mem_raddr_o = `KHOREE_ZeroWord;
        mem_waddr_o = `KHOREE_ZeroWord;
        mem_we = `KHOREE_WriteDisable;
        hold_flag = `KHOREE_HoldDisable;
        jump_flag = `KHOREE_JumpDisable;
        jump_addr = `KHOREE_ZeroWord;

        case (opcode)
            `KHOREE_INST_TYPE_I: begin
                case (funct3)
                    `KHOREE_INST_ADDI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = op1_add_op2_res;
                    end
                    `KHOREE_INST_SLTI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = {32{(~op1_ge_op2_signed)}} & 32'h1;
                    end
                    `KHOREE_INST_SLTIU: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = {32{(~op1_ge_op2_unsigned)}} & 32'h1;
                    end
                    `KHOREE_INST_XORI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = op1_i ^ op2_i;
                    end
                    `KHOREE_INST_ORI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = op1_i | op2_i;
                    end
                    `KHOREE_INST_ANDI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = op1_i & op2_i;
                    end
                    `KHOREE_INST_SLLI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = reg1_rdata_i << inst_i[24:20];
                    end
                    `KHOREE_INST_SRI: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        if (inst_i[30] == 1'b1) begin
                            reg_wdata = (sri_shift & sri_shift_mask) | ({32{reg1_rdata_i[31]}} & (~sri_shift_mask));
                        end else begin
                            reg_wdata = reg1_rdata_i >> inst_i[24:20];
                        end
                    end
                    default: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                    end
                endcase
            end
            `KHOREE_INST_TYPE_R_M: begin
                if ((funct7 == 7'b0000000) || (funct7 == 7'b0100000)) begin
                    case (funct3)
                        `KHOREE_INST_ADD_SUB: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            if (inst_i[30] == 1'b0) begin
                                reg_wdata = op1_add_op2_res;
                            end else begin
                                reg_wdata = op1_i - op2_i;
                            end
                        end
                        `KHOREE_INST_SLL: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = op1_i << op2_i[4:0];
                        end
                        `KHOREE_INST_SLT: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = {32{(~op1_ge_op2_signed)}} & 32'h1;
                        end
                        `KHOREE_INST_SLTU: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = {32{(~op1_ge_op2_unsigned)}} & 32'h1;
                        end
                        `KHOREE_INST_XOR: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = op1_i ^ op2_i;
                        end
                        `KHOREE_INST_SR: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            if (inst_i[30] == 1'b1) begin
                                reg_wdata = (sr_shift & sr_shift_mask) | ({32{reg1_rdata_i[31]}} & (~sr_shift_mask));
                            end else begin
                                reg_wdata = reg1_rdata_i >> reg2_rdata_i[4:0];
                            end
                        end
                        `KHOREE_INST_OR: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = op1_i | op2_i;
                        end
                        `KHOREE_INST_AND: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = op1_i & op2_i;
                        end
                        default: begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldDisable;
                            jump_addr = `KHOREE_ZeroWord;
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = `KHOREE_ZeroWord;
                        end
                    endcase
                end else begin
                    jump_flag = `KHOREE_JumpDisable;
                    hold_flag = `KHOREE_HoldDisable;
                    jump_addr = `KHOREE_ZeroWord;
                    mem_wdata_o = `KHOREE_ZeroWord;
                    mem_raddr_o = `KHOREE_ZeroWord;
                    mem_waddr_o = `KHOREE_ZeroWord;
                    mem_we = `KHOREE_WriteDisable;
                    reg_wdata = `KHOREE_ZeroWord;
                end
            end
            `KHOREE_INST_TYPE_L: begin
                case (funct3)
                    `KHOREE_INST_LB: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_raddr_o = op1_add_op2_res;
                        case (mem_raddr_index)
                            2'b00: begin
                                reg_wdata = {{24{mem_rdata_i[7]}}, mem_rdata_i[7:0]};
                            end
                            2'b01: begin
                                reg_wdata = {{24{mem_rdata_i[15]}}, mem_rdata_i[15:8]};
                            end
                            2'b10: begin
                                reg_wdata = {{24{mem_rdata_i[23]}}, mem_rdata_i[23:16]};
                            end
                            default: begin
                                reg_wdata = {{24{mem_rdata_i[31]}}, mem_rdata_i[31:24]};
                            end
                        endcase
                    end
                    `KHOREE_INST_LH: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_raddr_o = op1_add_op2_res;
                        if (mem_raddr_index == 2'b0) begin
                            reg_wdata = {{16{mem_rdata_i[15]}}, mem_rdata_i[15:0]};
                        end else begin
                            reg_wdata = {{16{mem_rdata_i[31]}}, mem_rdata_i[31:16]};
                        end
                    end
                    `KHOREE_INST_LW: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_raddr_o = op1_add_op2_res;
                        reg_wdata = mem_rdata_i;
                    end
                    `KHOREE_INST_LBU: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_raddr_o = op1_add_op2_res;
                        case (mem_raddr_index)
                            2'b00: begin
                                reg_wdata = {24'h0, mem_rdata_i[7:0]};
                            end
                            2'b01: begin
                                reg_wdata = {24'h0, mem_rdata_i[15:8]};
                            end
                            2'b10: begin
                                reg_wdata = {24'h0, mem_rdata_i[23:16]};
                            end
                            default: begin
                                reg_wdata = {24'h0, mem_rdata_i[31:24]};
                            end
                        endcase
                    end
                    `KHOREE_INST_LHU: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_raddr_o = op1_add_op2_res;
                        if (mem_raddr_index == 2'b0) begin
                            reg_wdata = {16'h0, mem_rdata_i[15:0]};
                        end else begin
                            reg_wdata = {16'h0, mem_rdata_i[31:16]};
                        end
                    end
                    default: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                    end
                endcase
            end
            `KHOREE_INST_TYPE_S: begin
                case (funct3)
                    `KHOREE_INST_SB: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        reg_wdata = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteEnable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_waddr_o = op1_add_op2_res;
                        mem_raddr_o = op1_add_op2_res;
                        case (mem_waddr_index)
                            2'b00: begin
                                mem_wdata_o = {mem_rdata_i[31:8], reg2_rdata_i[7:0]};
                            end
                            2'b01: begin
                                mem_wdata_o = {mem_rdata_i[31:16], reg2_rdata_i[7:0], mem_rdata_i[7:0]};
                            end
                            2'b10: begin
                                mem_wdata_o = {mem_rdata_i[31:24], reg2_rdata_i[7:0], mem_rdata_i[15:0]};
                            end
                            default: begin
                                mem_wdata_o = {reg2_rdata_i[7:0], mem_rdata_i[23:0]};
                            end
                        endcase
                    end
                    `KHOREE_INST_SH: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        reg_wdata = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteEnable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_waddr_o = op1_add_op2_res;
                        mem_raddr_o = op1_add_op2_res;
                        if (mem_waddr_index == 2'b00) begin
                            mem_wdata_o = {mem_rdata_i[31:16], reg2_rdata_i[15:0]};
                        end else begin
                            mem_wdata_o = {reg2_rdata_i[15:0], mem_rdata_i[15:0]};
                        end
                    end
                    `KHOREE_INST_SW: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        reg_wdata = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteEnable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_waddr_o = op1_add_op2_res;
                        mem_raddr_o = op1_add_op2_res;
                        mem_wdata_o = reg2_rdata_i;
                    end
                    default: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                    end
                endcase
            end
            `KHOREE_INST_TYPE_EXT: begin
                case (funct3)
                    `KHOREE_INST_SID: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        reg_wdata = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteEnable;
                        mem_req = `KHOREE_RIB_REQ;
                        mem_waddr_o = op1_add_op2_res;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_wdata_o = 32'h00000005;
                    end
                    `KHOREE_INST_RT: begin
                        if (mem_ack_flag) begin
                            hold_flag = `KHOREE_HoldDisable;
                            mem_req = `KHOREE_RIB_NREQ;
                        end
                        else begin
                            jump_flag = `KHOREE_JumpDisable;
                            hold_flag = `KHOREE_HoldEnable;
                            jump_addr = `KHOREE_ZeroWord;
                            reg_wdata = mem_rdata_i;
                            mem_we = `KHOREE_WriteDisable;
                            mem_req = `KHOREE_RIB_REQ;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_raddr_o = op1_add_op2_res;
                            mem_wdata_o = `KHOREE_ZeroWord;
                        end
                    end
                    `KHOREE_INST_IF: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        if (inst_i[31:20] == 12'b0) begin
                            if (op1_ge_op2_signed) begin
                                mem_wdata_o = {24'b0, op1_i[7:0]};
                                mem_raddr_o = `KHOREE_ZeroWord;
                                mem_waddr_o = 32'h3000000c;
                                mem_req = `KHOREE_RIB_REQ;
                                mem_we = `KHOREE_WriteEnable;
                                reg_wdata = `KHOREE_ZeroWord;
                            end
                            else begin
                                mem_wdata_o = `KHOREE_ZeroWord;
                                mem_raddr_o = `KHOREE_ZeroWord;
                                mem_waddr_o = `KHOREE_ZeroWord;
                                mem_we = `KHOREE_WriteDisable;
                                reg_wdata = op1_i;
                            end
                        end
                        else begin
                            mem_wdata_o = `KHOREE_ZeroWord;
                            mem_raddr_o = `KHOREE_ZeroWord;
                            mem_waddr_o = `KHOREE_ZeroWord;
                            mem_we = `KHOREE_WriteDisable;
                            reg_wdata = op1_add_op2_res;
                        end
                    end
                    default: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                    end
                endcase
            end
            `KHOREE_INST_TYPE_B: begin
                case (funct3)
                    `KHOREE_INST_BEQ: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = op1_eq_op2 ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    `KHOREE_INST_BNE: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = (~op1_eq_op2) ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    `KHOREE_INST_BLT: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = (~op1_ge_op2_signed) ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    `KHOREE_INST_BGE: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = (op1_ge_op2_signed) ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    `KHOREE_INST_BLTU: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = (~op1_ge_op2_unsigned) ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    `KHOREE_INST_BGEU: begin
                        hold_flag = `KHOREE_HoldDisable;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                        jump_flag = (op1_ge_op2_unsigned) ^ prdt_taken_i;
                        jump_addr = op1_jump_add_op2_jump_res;
                    end
                    default: begin
                        jump_flag = `KHOREE_JumpDisable;
                        hold_flag = `KHOREE_HoldDisable;
                        jump_addr = `KHOREE_ZeroWord;
                        mem_wdata_o = `KHOREE_ZeroWord;
                        mem_raddr_o = `KHOREE_ZeroWord;
                        mem_waddr_o = `KHOREE_ZeroWord;
                        mem_we = `KHOREE_WriteDisable;
                        reg_wdata = `KHOREE_ZeroWord;
                    end
                endcase
            end
            `KHOREE_INST_JAL: begin
                hold_flag = `KHOREE_HoldDisable;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                jump_flag = `KHOREE_JumpDisable;
                jump_addr = op1_jump_add_op2_jump_res;
                reg_wdata = op1_add_op2_res;
            end
            `KHOREE_INST_JALR: begin
                hold_flag = `KHOREE_HoldDisable;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                jump_flag = `KHOREE_JumpEnable;
                jump_addr = op1_jump_add_op2_jump_res;
                reg_wdata = op1_add_op2_res;
            end
            `KHOREE_INST_LUI, `KHOREE_INST_AUIPC: begin
                hold_flag = `KHOREE_HoldDisable;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                jump_addr = `KHOREE_ZeroWord;
                jump_flag = `KHOREE_JumpDisable;
                reg_wdata = op1_add_op2_res;
            end
            `KHOREE_INST_NOP_OP: begin
                jump_flag = `KHOREE_JumpDisable;
                hold_flag = `KHOREE_HoldDisable;
                jump_addr = `KHOREE_ZeroWord;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                reg_wdata = `KHOREE_ZeroWord;
            end
            `KHOREE_INST_FENCE: begin
                hold_flag = `KHOREE_HoldDisable;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                reg_wdata = `KHOREE_ZeroWord;
                jump_flag = `KHOREE_JumpEnable;
                jump_addr = op1_jump_add_op2_jump_res;
            end
            default: begin
                jump_flag = `KHOREE_JumpDisable;
                hold_flag = `KHOREE_HoldDisable;
                jump_addr = `KHOREE_ZeroWord;
                mem_wdata_o = `KHOREE_ZeroWord;
                mem_raddr_o = `KHOREE_ZeroWord;
                mem_waddr_o = `KHOREE_ZeroWord;
                mem_we = `KHOREE_WriteDisable;
                reg_wdata = `KHOREE_ZeroWord;
            end
        endcase

        if (mem_req == `KHOREE_RIB_REQ && mem_ack_i == `KHOREE_RIB_NACK) begin
            hold_flag = `KHOREE_HoldEnable;
        end
    end

endmodule
