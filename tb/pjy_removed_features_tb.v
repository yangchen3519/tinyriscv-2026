...
module pjy_removed_features_tb;
    ...
    wire[31:0] op1,op2,op1j,op2j,inst_o,inst_addr_o,rdata1_o,rdata2_o;
    wire reg_we;
    integer errors=0;
`ifdef FSDB
...
`endif
    task rejected; input[31:0] code; input[127:0] name; begin
        ...
        if(reg_we!==1'b0) begin
            $display("FAIL removed feature %0s reg_we=%b",name,reg_we); errors=errors+1;
        end
    end endtask
    initial begin
    ...
    pjy_id dut(.rst(rst),.inst_i(inst),.inst_addr_i(32'b0),.reg1_rdata_i(32'h11),.reg2_rdata_i(32'h22),
        .ex_jump_flag_i(1'b0),.reg1_raddr_o(raddr1),.reg2_raddr_o(raddr2),
        .op1_o(op1),.op2_o(op2),.op1_jump_o(op1j),.op2_jump_o(op2j),
        .inst_o(inst_o),.inst_addr_o(inst_addr_o),.reg1_rdata_o(rdata1_o),.reg2_rdata_o(rdata2_o),
        .reg_we_o(reg_we),.reg_waddr_o(waddr));
endmodule
